    ...
    <main class="flex-grow py-12 px-4">
        <div class="max-w-2xl mx-auto">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-[#d1d5db] p-6 sm:p-8 mb-8">
                <form action="{{ route('tracking.barang.form') }}" method="POST" class="flex flex-col sm:flex-row gap-3">
                    @csrf
                    <input 
                        type="text" 
                        name="kode" 
                        value="{{ $kode ?? old('kode') }}"
                        placeholder="Masukkan kode peminjaman, contoh: PB4B-080826-B4B-001-001"
                ...
                {{-- Tampilkan Error Jika Kode Tidak Ditemukan --}}
                @error('kode')
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-[#d1d5db] p-6 sm:p-8">
                    ...
                    <dl class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                        ...
                        <div class="sm:col-span-2">
                            <dt class="text-[#555555] font-medium mb-2">Daftar Barang</dt>
                            <dd class="text-[#333333]">
                                <ul class="list-disc pl-5 space-y-1">
                                    @foreach($peminjaman->details as $detail)
                                        <li>{{ $detail->barang->nama_barang }} — <strong>{{ $detail->jumlah }}</strong> unit</li>
                                    @endforeach
                                </ul>
                            </dd>
                        </div>
                        <div>
                            <dt class="text-[#555555] font-medium">Tanggal Mulai</dt>
                            <dd class="text-[#333333]">{{ \Carbon\Carbon::parse($peminjaman->tanggal_mulai)->format('d/m/Y') }}</dd>
                        </div>
                        <div>
                            <dt class="text-[#555555] font-medium">Tanggal Selesai</dt>
                            <dd class="text-[#333333]">{{ \Carbon\Carbon::parse($peminjaman->tanggal_selesai)->format('d/m/Y') }}</dd>
                        </div>
                        <div>
                        ...
                        @if($peminjaman->status == 'dikembalikan' && $peminjaman->pengembalians->count() > 0)
                        <div class="sm:col-span-2 bg-gray-50 p-4 rounded-lg mt-2">
                            ...
                            @foreach($peminjaman->pengembalians as $pengembalian)
                                <div class="grid grid-cols-2 gap-2 text-sm mb-2">
                                    <div>
                                        ...
                                        <dd class="text-[#333333]">{{ \Carbon\Carbon::parse($pengembalian->tanggal_pengembalian)->format('d/m/Y') }}</dd>
                                    </div>
                                    <div>
                                        <dt class="text-[#555555] font-medium">Jumlah Kembali</dt>
                                        <dd class="text-[#333333]">{{ $pengembalian->jumlah_kembali }}</dd>
                                    </div>
                                    <div>
                                        <dt class="text-[#555555] font-medium">Hilang</dt>
                                        <dd class="text-[#333333]">{{ $pengembalian->jumlah_hilang }}</dd>
                                    </div>
                                    <div>
                                        <dt class="text-[#555555] font-medium">Keterangan</dt>
                                        <dd class="text-[#333333]">{{ $pengembalian->keterangan ?? '-' }}</dd>
                                    </div>
                                </div>
                            @endforeach
                        </div>
                    ...
                    @if($peminjaman->status == 'disetujui')
                        ...
                        <div class="mt-6 text-center">
                            <a href="{{ route('surat.barang', $peminjaman->kode_peminjam) }}" 
                            class="inline-block bg-[#eab308] hover:bg-[#facc15] text-[#0f213d] font-semibold px-6 py-3 rounded-lg transition shadow">
                                <i class="fa-solid fa-download mr-2"></i> Unduh Surat Permohonan Barang (PDF)
                            </a>
                        </div>
                    ...
                    @elseif($peminjaman->status == 'dikembalikan')
                        <div class="mt-6 bg-blue-50 border border-blue-200 text-blue-800 rounded-lg p-4">
                            <i class="fa-solid fa-flag-checkered mr-2"></i> Barang telah <strong>Dikembalikan</strong>.
                        </div>
                    @else
